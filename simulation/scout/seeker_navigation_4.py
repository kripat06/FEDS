...
def go_north(stop_distance):
    reached = False
    current_distance = get_distance(FRONT_SENSOR_IDX)
    drive_forward()                
    while not reached:
...
def go_east(stop_distance):
    reached = False
    current_distance = get_distance(LEFT_B_SENSOR_IDX)
    slide_right()
    while not reached:
        if current_distance >= stop_distance:
            brake()
            reached = True
            #print("East CD: "+str(current_distance) + " ")
        else:
            current_distance = get_distance(LEFT_B_SENSOR_IDX)
            #print("East CD: "+str(current_distance) + " ")
...
def ping_to_take_picture():
    display.text("TAKING PICTURE", 0, 10, 10)
    display.show()
    time.sleep(2)
    display.fill(0)
    display.show()
...
while go:
    ...
    ping_to_take_picture()

    go_north(20)
    front_self_correct(0.1, 20)
    front_self_correct(0.1, 20)
    self_correct(0.1)
    self_correct(0.1)
    lateral_self_correct(0.1, 5.5)
    ...
    self_correct(0.1)
    self_correct(0.1)
    lateral_self_correct(0.1, 31)
    lateral_self_correct(0.1, 31)
    self_correct(0.1)
    self_correct(0.1)
    front_self_correct(0.1, 30.5)
    front_self_correct(0.1, 30.5)
    self_correct(0.1)
    self_correct(0.1)
