...
# the sensor distance readings for all available sensors.
    for sensor_number, sensor in enumerate(vl53l4cd):
        if sensor.data_ready:
            #print("Sensor {}: {}".format(sensor_number + 1, sensor.distance))
            sensor.clear_interrupt()
            distance_readings.append(sensor.distance)
    return distance_readings

time.sleep(2)
while True:
    sensor_data = get_sensor_data()
    #print("F: " + str(sensor_data[1]) + "cm")
   # print("LF: " + str(sensor_data[2]) + "cm")
    print("LB: " + str(sensor_data[0]) + "cm")
    print("----------------------")
    time.sleep(2)
